...
async def client_do_listen(bind_host, bind_port):#client连接listen
    sock_port = '-----'
    try:
        ...
        writer.write(client_data)#client向8001（listen)写数据
        data = await reader.readexactly(len(server_data))
        if data != server_data:#接受的应该是server_data
            log.error('S L R>C data {:5} > {:5} {} error server_data {}'.format(sock_port, bind_port, data, server_data))#不是的话就出错
            writer.close()
            return

        log.info('S L R>C data {:5} > {:5} {}'.format(sock_port, bind_port, data))#client接受到了listen发送来的数据
...
async def server_do_slave(reader, writer):#server去连接slave
    peer_host, peer_port, = writer.get_extra_info('peername')
    sock_host, sock_port, = writer.get_extra_info('sockname')
    log.info('S<L R C open {:5} < {:5}'.format(sock_port, peer_port))#server连接slave的提示

    try:
        log.info('S>L R C data {:5} > {:5} {}'.format(sock_port, peer_port, server_data))
        writer.write(server_data)#slave向server写数据（server_data）
        data = await reader.readexactly(len(client_data))#等待接受来自server发送来的数据
        if data != client_data:
...
loop.run_until_complete(asyncio.gather(*[client_do_listen(bind_host, bind_port) for i in range(args.test_times)]))#连续多次连接8001
